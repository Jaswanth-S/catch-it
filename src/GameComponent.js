...
import {Link} from 'react-router-dom';
  ...
  class GameComponent extends Component {
    ...
    startGame() {
      this.removeTimers();
      this.moveSnakeInterval = setInterval(this.moveSnake, 200);
      this.moveFood();
    ...
    render() {
      // each cell should be approximately 20px wide, so calculate how many we need
      ...
      if (this.state.status === 0) {
        ...
        overlay = (
          <div>
            ...
            <Link className="btn btn-info lin" to="/leaderboard">leaderboard</Link>
            </button>
        </div>
        );
      }
      ...
      overlay,
      React.createElement("div", { className: "mb-1 colr" }, "score: ", this.state.snake.length, " "),
      // React.createElement("button",{className:"btn btn-info "},"leaderboard"),
      React.createElement("div", {
        className: "grid",
        style: {
          width: this.props.size + "px",
...
//ReactDOM.render(<GameComponent size={350}/>,document.getElementById('root'))
